...
def plot_embeddings(emb, emb_l, labels, filepath):
    ...
    plt.figure()
    plt.scatter(emb[:, 0], emb[:, 1], c=emb_l, cmap=cmap_obj, s=0.5, alpha=0.2, edgecolors='none')
...
def plot_kls_df(df, filepath, yscale):
    ...
    with sns.plotting_context("notebook", font_scale=2.0):
        ...
        g = g.map(sns.boxplot, df.columns[1], df.columns[2], df.columns[0], palette=cmap_arr[1:], showfliers=False,
