...
dim(filtered_count) #5000 161764

filtered_barcode <- matrix(colnames(filtered_count), ncol=1)
dim(filtered_barcode) #161764 1
...
#Toy data
dir.create("data/Seurat_v4_toy/filtered_RNA-seq")
data <- readMM(file="data/Seurat_v4_toy/RNA-seq/RNA_count.mtx")
filtered_count <- data[match(VariableFeatures(d), rownames(d@assays$RNA@counts)),]
dim(filtered_count) #5000 10000

writeMM(filtered_count,file="data/Seurat_v4_toy/filtered_RNA-seq/RNA_count.mtx")
write.table(filtered_gene,file="data/Seurat_v4_toy/filtered_RNA-seq/gene.tsv",sep="\t",row.names=FALSE,col.names=FALSE)
